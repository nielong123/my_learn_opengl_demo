...
#include "matrixUtil/Vec2.h"
#include "matrixUtil/MathUtil.h"
...
bool isOneDimensionSegmentOverlap(float A, float B, float C, float D, float *S, float * E)
{
    ...
    else
    {
        if (ABmin >= CDmin && ABmin <= CDmax)
        {
            ...
            if (S != NULL) *S = ABmin;
            if (E != NULL) *E = CDmax < ABmax ? CDmax : ABmax;
        }
                            ...
                            float *S, float *T)
{
    ...
    if (S != NULL) *S = crossProduct2Vector(C, D, C, A) / denom;
    if (T != NULL) *T = crossProduct2Vector(A, B, C, A) / denom;
